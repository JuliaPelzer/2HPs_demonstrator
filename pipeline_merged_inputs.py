...
class Domain:
    ...
    def extract_hp_boxes(self):
        ...
        for idx in range(len(pos_hps)):
            ...
            corner_ll, corner_ur = get_box_corners(
                pos_hp, size_hp_box, distance_hp_corner, self.inputs.shape[1:]
            )
    ...
    def overwrite_boxes_prediction_1HP(self, hp: "HeatPump"):
        ...
        corner_ll, corner_ur = get_box_corners(
            hp.pos, hp.prediction_1HP.shape, hp.dist_corner_hp, self.prediction.shape
        )
        ...
        self.info["Inputs"]["Original Temperature [C]"] = self.info["Labels"][
            "Temperature [C]"
        ].copy()
        self.info["Inputs"]["Original Temperature [C]"][
            "index"
        ] = tmp_index  # keep the index!
...
class HeatPump:
    def __init__(
        self, id, pos, orientation, inputs=None, dist_corner_hp=None, label=None
    ):
