...
class Domain:
    def __init__(self, info_path:str, stitching_method:str="max", file_name:str="RUN_0.pt"):
        ...
        if self.get_input_field_from_name('Permeability X [m^2]').max() > 1 or self.get_input_field_from_name('Permeability X [m^2]').min() < 0:
            ...
            shutil.move(os.path.join(origin_2hp_prep, "Inputs", file_name), os.path.join(origin_2hp_prep, "broken", "Inputs", f"{file_name.split('.')[0]}_perm_outside_0_1.pt"))
            shutil.move(os.path.join(origin_2hp_prep, "Labels", file_name), os.path.join(origin_2hp_prep, "broken", "Labels", file_name))
            beep()
        assert self.get_input_field_from_name('Permeability X [m^2]').max() <= 1, f"Max of permeability X [m^2] not < 1 but {self.get_input_field_from_name('Permeability X [m^2]').max()} for {file_name}"
        assert self.get_input_field_from_name('Permeability X [m^2]').min() >= 0, f"Min of permeability X [m^2] not > 0 but {self.get_input_field_from_name('Permeability X [m^2]').min()} for {file_name}"
        ...
        try:
            ...
            p_related_field = self.get_input_field_from_name(p_related_name)
        logging.info(f"{p_related_name} in range ({p_related_field.max()}, {p_related_field.min()})")

        if p_related_field.max() > 1 or p_related_field.min() < 0:
            origin_2hp_prep = "/home/pelzerja/Development/datasets_prepared/2hps_demonstrator/dataset_2hps_1fixed_1000dp_grad_p"
            # TODO rm absolute path
            shutil.move(os.path.join(origin_2hp_prep, "Inputs", file_name), os.path.join(origin_2hp_prep, "broken", "Inputs", f"{file_name.split('.')[0]}_p_outside_0_1.pt"))
            shutil.move(os.path.join(origin_2hp_prep, "Labels", file_name), os.path.join(origin_2hp_prep, "broken", "Labels", file_name))
            beep()
...
class HeatPump:
    def __init__(self, id, pos, orientation, inputs, dist_corner_hp=None, label=None):
        ...
        self.pos:list = np.array([int(pos[0]), int(pos[1])])    #(x,y), cell-ids # TODO(y,x)??
        ...
        self.interim_outputs:np.ndarray = np.ones(self.inputs[0].shape, dtype="float32") *10.6 # input for 2HP-NN, size of one input
        self.label = label
    ...
    def get_inputs_for_2hp_nn(self, single_hps):
        ...
        for hp in single_hps:
            # get other hps
            if hp.id != self.id:
                assert (self.interim_outputs.shape == hp.prediction_1HP.shape), "Shapes don't fit - line 241"
                ...
                self.interim_outputs[offset2[0]:end2[0], offset2[1]:end2[1]] = np.max([self.interim_outputs[offset2[0]:end2[0], offset2[1]:end2[1]], tmp_2nd_hp], axis=0)
    ...
    def plot(self, dir:str="HP-Boxes", data_to_plot:np.ndarray=None, names:np.ndarray=None):
...
def pipeline_prepare_separate_inputs_for_2HPNN(dataset_large_name:str, model_name_1HP:str, dataset_trained_model_name:str, input_pg:str, model_name_2HP:str=None, device:str="cuda:0"):
    """
    assumptions:
    ...
    datasets_raw_domain_dir, datasets_prepared_domain_dir, dataset_domain_path, datasets_model_trained_with_path, model_1hp_path, _, name_extension, datasets_prepared_2hp_dir = set_paths(dataset_large_name, model_name_1HP, dataset_trained_model_name, input_pg, model_name_2hp=model_name_2HP)
    ...
    for run_file in tqdm(list_runs, desc="2HP prepare", total=len(list_runs)):
        ...
        for hp in single_hps:
            ...
            hp.save(run_id=run_id, dir=destination_2hp_prep, inputs_all=np.array([hp.prediction_1HP, hp.interim_outputs]))
    ...
    save_config_of_separate_inputs(domain.info, path=destination_2hp_prep, name_file="info")
    # save command line arguments
    cla = {"dataset_large_name":dataset_large_name,
...
def save_config_of_separate_inputs(domain_info, path, name_file="info"):
    ...
    shortened_input = {"Temperature prediction (1st HP) [C]": temperature_info.copy(),
    ...
    shortened_infos = {"Inputs": shortened_input,
...
if __name__ == "__main__":
    ...
    pipeline_prepare_separate_inputs_for_2HPNN(dataset_large_name=args.dataset_large, model_name_1HP=args.model, dataset_trained_model_name=args.dataset_boxes, input_pg=args.input_pg, model_name_2HP=args.model_2hp, device=args.device)
